...
int main(int argc, char *argv[]) {
    ...
    dungeon->running = true; // sets dungeon to running = true
    
    sem_t *sem_one = sem_open(dungeon_lever_one, O_CREAT | O_EXCL, S_IRUSR | S_IWUSR, 0); // creates semaphore for lever one
    sem_t *sem_two = sem_open(dungeon_lever_two, O_CREAT | O_EXCL, S_IRUSR | S_IWUSR, 0); // creates semaphore for lever two
    ...
    sem_close(sem_one); // closes semaphore to lever one
    sem_close(sem_two); // closes semaphore to lever two
